...
#     def genAnchorBoxes(self, featmapSize, stride=170):
#         feat_w, feat_h = featmapSize
#         shift_x = torch.arange(0, feat_w, device=self.device).repeat(feat_h).reshape(feat_w, feat_h).to(self.device)
#         shift_y = torch.t(shift_x)
#
#         allAnchors = torch.clone(self.baseAnchors.to(self.device))
#         alla = torch.zeros((feat_w,feat_h, len(self.wsizes), 4)).to(self.device)
