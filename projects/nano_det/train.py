...
"""
            ...
            for level in range(headerNum):
                ...
                if len(posindex) > 0:
                    """giou loss (posNum, )"""
                    giouLossLevel = GIoULoss()(posBoxGtAnchorLevel, posBoxPredLevelLUOne)
                    ...
                    dfLossLevel = DistributionFocalLoss(reduction='none').cal(a, b)  # 这里一共8个位置， 每个点距离anchor点可能是0-7，共8个距离，
                else:
                    giouLossLevel = boxPredLevel.sum() * 0
                    dfLossLevel = boxPredLevel.sum() * 0
                ...
                qfLossLevel = QualityFocalLoss().cal(clsPredLevel, (clsGtAnchorLevel, quality))

                # batchPosNum = torch.nonzero((torch.hstack(clsGtAnchor) + 1).reshape(-1)).numel()
                batchPosNum = torch.nonzero((torch.cat(clsGtAnchor, dim=1) + 1).reshape(-1)).numel()

                qfLossLevel = qfLossLevel.sum() / batchPosNum

                loss.append([giouLossLevel.sum(), len(posindex)])
                loss.append([dfLossLevel.sum(), len(posindex)])
                loss.append([qfLossLevel, batchPosNum])
