...
class ImgAugWithShape(object):
    def __init__(self, img, boxes=None, mask=None):
        self.img = img
        self.boxes = boxes
        self.mask = mask
    ...
    def fliplr(self, prob = 0.5):
        ...
        if self.mask is not None:
            self.mask = cv2.warpPerspective(self.mask, M, dsize=None)
    ...
    def translate(self, translate=3, prob=0.5):
        ...
        if self.boxes is not None:
            self.boxes = self.warpBoxes(M)
        if self.mask is not None:
            self.mask =  self.mask = cv2.warpPerspective(self.mask, M, dsize=None)
